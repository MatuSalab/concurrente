...
public class Main {
     ...
     * Thread hiloEstudiante = new Thread(new Estudiante(cafetera, i+"", pedido,
     * aviso, mutex), "Estudiante-" + i);
     ...
     * for (int i = 0; i < tenedores.length; i++) {
     * Semaphore izq= tenedores[i];
     * Semaphore der= tenedores[(i+1)%tenedores.length]; //F1 toma tenedores 0(i) y
     * 1(d). F2 toma tenedores 1(i) y 2(d).
     * //F3 toma tenedores 2(i) y 3(d). F4 toma tenedores 3(i) y 4(d). F5 toma
     * tenedores 4(i) y 0(d)
     * 
     * new Thread(new Filosofo(i+1,izq, der)).start();
     * }
     ...
     * 
     * public static void main(String[] args) {
     * Semaphore mutexLeer = new Semaphore(1);
     * Semaphore mutexEscribir = new Semaphore(1);
     * Semaphore mutex1 = new Semaphore(1);
     * Semaphore mutex2 = new Semaphore(1);
     * Libro libro = new Libro(mutexLeer, mutexEscribir, mutex1, mutex2);
     * new Thread(new Escritor(1, libro)).start();
     * new Thread(new Escritor(2, libro)).start();
     * for (int i = 1; i <= 10; i++) {
     * new Thread(new Lector(i, libro)).start();
     * }
     * 
     * 
     * }
